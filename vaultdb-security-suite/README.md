...
publica os executáveis (Windows/macOS) e os binários do agente Go numa GitHub Release.
...
### v2.2 — novos módulos
- **Agente Go (`agent/`):** binário único/estático que dispara backups via Director API; unit systemd + Makefile.
- **XtraBackup incremental:** `POST /api/backups/xtrabackup` e `GET /api/backups/xtrabackup/script`.
- **Mascaramento LGPD:** `mask_lgpd` anonimiza colunas sensíveis (MD5) após o restauro.
- **Sandbox Proxmox:** `POST /api/sandbox/proxmox` (LXC isolado via API) + script `pct exec` de validação.
- **Exportação Parquet/S3:** `POST /api/export/parquet` (pyarrow + boto3).
- **Multi-tenant:** `tenant_id` em conexões/utilizadores, scoping e gestão no menu Configurações.
- **E-mail de aprovação** em HTML responsivo.
